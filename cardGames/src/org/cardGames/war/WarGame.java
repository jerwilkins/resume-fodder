...
public class WarGame{
	...
	WarGame( int numberOfSuits, int numberOfRanks, int numberOfPlayers ) {
		...
		warDeck = new CardDeck(numberOfSuits, numberOfRanks);
	...
	private Player determineBattleWinner(LinkedList<Player> playersThisBattle, LinkedList<Player> tiedPlayers) {
		//LinkedList<Player> = new LinkedList<Player>();
...
//	REMOVED CODE AS LIKELY UNREACHABLE		
// TODO: remove altogether after 2012-01-31			
//			if(player.getCardsInPlay().isEmpty()){
//				this.cardsInLimbo.addAll(player.getCardsInJeopardy());
//				player.getCardsInJeopardy().clear(); // TODO will this ever execute?
//				continue;
//			}
//			else {
...
//			}
			...
			// if player loses, send all their cards to limbo
				...
				this.cardsInLimbo.addAll(player.getCardsInJeopardy());
				player.getCardsInJeopardy().clear();
				player.getCardsInPlay().clear(); // TODO debugger has error retrieving this from stack frame
			}
		}
		return winningPlayer;
	}
